...
class MH_Typechecker {
    ...
    static MH_TYPE computeType (MH_EXP exp, TYPE_ENV env)
        ...
        if (exp.isVAR()) {
            ...
            if(!exp.value().matches("^[0-9]+$")) { // Checks with a regex if value is an Integer
                throw new TypeError ("Invalid integer: " + exp.value()) ;
            }
            return IntegerType;
        } else if(exp.isBOOLEAN()) {
            if(!exp.value().equals("True") && !exp.value().equals("False")) {
                throw new TypeError ("Invalid boolean: " + exp.value()) ;
            }
            return BoolType;
        } else if(exp.isINFIX()) {
            if(!computeType(exp.first(), env).equals(IntegerType) || !computeType(exp.second(), env).equals(IntegerType)) {
                throw new TypeError ("The arguments for the infix operation are not integers!") ;
            }
            ...
            MH_TYPE first = computeType(exp.first(), env);  // Type of the function application's first expression
            MH_TYPE second = computeType(exp.second(), env);  // Type of the function application's second expression

            if(!first.left().equals(second)) {
                throw new TypeError ("The type of the second argument does not match the type of left side of the first one in the apllication operation!") ;
            }
            return first.right();
        } else if(exp.isIF()) {
            MH_TYPE first = computeType(exp.first(), env);  // Type of the if condition
            if(!first.equals(BoolType)) {
                throw new TypeError ("The type of the first argument of the if operation is not bool!") ;
            }
            MH_TYPE second = computeType(exp.second(), env);  // Type of the expression calculated when the condition is True
            MH_TYPE third = computeType(exp.third(), env);    // Type of the expression calculated when the condition is False
            if(!second.equals(third)) {
                throw new TypeError("Branches of the if operation must be of the same type. Expecting a " + second.toString() + ", but found " + third.toString());
            }
            return third;   // can return either second or third since they're the same type
        }
        else {
